...
class NTMHeadBase(nn.Module):
    ...
    def _address_memory(self, k, β):
        # Handle Activations
        k = k.clone()
        β = F.softplus(β)

        w = self.memory.address(k, β)
...
class NTMReadHead(NTMHeadBase):
    ...
    def forward(self, embedding):
        ...
        o = self.fc_read(embedding)
        k, β = _split_cols(o, self.read_lengths)

        # Read from memory
        w = self._address_memory(k, β)
        print ("Read weight", w, "Beta", β)
...
class NTMWriteHead(NTMHeadBase):
    ...
    def forward(self, embeddings):
        ...
        k, β, e, a = _split_cols(o, self.write_lengths)
